...
bool get_object_sse_type(const char* path, sse_type_t& ssetype, string& ssevalue)
{
  ...
  for(headers_t::iterator iter = meta.begin(); iter != meta.end(); ++iter){
    ...
    if(0 == strcasecmp(key.c_str(), "x-cos-server-side-encryption") && 0 == strcasecmp((*iter).second.c_str(), "AES256")){
      ssetype  = SSE_OSS;
    }else if(0 == strcasecmp(key.c_str(), "x-cos-server-side-encryption-customer-key-md5")){
      ssetype  = SSE_C;
...
static int create_directory_object(const char* path, mode_t mode, time_t time, uid_t uid, gid_t gid)
{
  ...
  meta["x-cos-meta-gid"]   = str(gid);
  meta["x-cos-meta-mode"]  = str(mode);
  meta["x-cos-meta-mtime"] = str(time);
...
static int s3fs_symlink(const char* from, const char* to)
{
  ...
  headers["x-cos-meta-mode"]  = str(S_IFLNK | S_IRWXU | S_IRWXG | S_IRWXO);
  headers["x-cos-meta-mtime"] = str(time(NULL));
  headers["x-cos-meta-uid"]   = str(pcxt->uid);
  headers["x-cos-meta-gid"]   = str(pcxt->gid);
...
static int s3fs_fsync(const char* path, int datasync, struct fuse_file_info* fi)
{
  ...
  // Delete stat cache entry because st_size may have changed.
  StatCache::getStatCacheData()->DelStat(path);
...
static int set_xattrs_to_header(headers_t& meta, const char* name, const char* value, size_t size, int flags)
{
  ...
  meta["x-cos-meta-xattr"] = build_xattrs(xattrs);
...
static int check_passwd_file_perms(void)
{
  ...
  if(passwd_file != "/etc/passwd-cosfs"){
    ...
    // "/etc/passwd-cosfs" does not allow group write.
...
static int get_access_keys(void)
{
  ...
  char* COSACCESSKEYID     = getenv("COSACCESSKEYID");
  char* COSSECRETACCESSKEY = getenv("COSSECRETACCESSKEY");
  if(COSACCESSKEYID != NULL || COSSECRETACCESSKEY != NULL){
    if( (COSACCESSKEYID == NULL && COSSECRETACCESSKEY != NULL) ||
        (COSACCESSKEYID != NULL && COSSECRETACCESSKEY == NULL) ){
      S3FS_PRN_EXIT("if environment variable OSSACCESSKEYID is set then OSSSECRETACCESSKEY must be set too.");
      return EXIT_FAILURE;
    }
    if(!S3fsCurl::SetAccessKey(COSACCESSKEYID, COSSECRETACCESSKEY)){
      S3FS_PRN_EXIT("if one access key is specified, both keys need to be specified.");
      return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
  ...
  char * COS_CREDENTIAL_FILE;
  COS_CREDENTIAL_FILE = getenv("COS_CREDENTIAL_FILE");
  if(COS_CREDENTIAL_FILE != NULL){
    passwd_file.assign(COS_CREDENTIAL_FILE);
    if(passwd_file.size() > 0){
      ifstream PF(passwd_file.c_str());
      if(PF.good()){
         PF.close();
  ...
  if(HOME != NULL){
     ...
     passwd_file.append("/.passwd-cosfs");
     ifstream PF(passwd_file.c_str());
     if(PF.good()){
       PF.close();
